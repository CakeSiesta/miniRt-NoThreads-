...
/*   Updated: 2020/09/14 12:08:00 by mkravetz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		handle_key(int k, t_mlx *my_mlx)
{
	...
	if (!(k >= 0 && k <= 3) && !(k >= 12 && k <= 15) &&
		...
		return (0);
	o->x += ((k == KEYCODE_A) ? -0.5 : 0) + ((k == KEYCODE_D) ? 0.5 : 0);//move left/right
	o->y += ((k == KEYCODE_S) ? -0.5 : 0) + ((k == KEYCODE_W) ? 0.5 : 0);//move down/up
	o->z += ((k == KEYCODE_F) ? -0.5 : 0) + ((k == KEYCODE_R) ? 0.5 : 0); //move forward/backwards
	r->x += ((k == KEYCODE_DOWN) ? -0.1 : 0) + ((k == KEYCODE_UP) ? 0.1 : 0);//move left/right
	r->y += ((k == KEYCODE_LEFT) ? -0.1 : 0) + ((k == KEYCODE_RIGHT) ? 0.1 : 0);//move down/up
	r->z += ((k == KEYCODE_1) ? -0.1 : 0) + ((k == KEYCODE_2) ? 0.1 : 0);//turn camera counterclockwise/clockwise
	if (k == KEYCODE_Q)
